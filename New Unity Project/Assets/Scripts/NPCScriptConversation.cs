...
public class NPCScriptConversation : MonoBehaviour
{
    ...
    public PlayerResponseHandler playerResponseHandler;
    public GameObject inputPanel; // Reference to the dialogue panel UI


    private void Start()
    {
        dialoguePanel.SetActive(false); // Make sure the dialogue panel is inactive at the start
        inputPanel.SetActive(false); // Make sure the dialogue panel is inactive at the start
    }
    ...
    {
        playerResponseHandler.ResetResponse(); // Reset the response at the start of the conversation

        DialogueEntry[] conversation = new DialogueEntry[]
    {
        new DialogueEntry { speakerName = "Female Orc", message = "Whoa, you're still in one piece? Color me impressed!" },
        new DialogueEntry { speakerName = "Female Orc", message = "You've tiptoed through terror and danced with danger." },
        new DialogueEntry { speakerName = "Female Orc", message = "If you had to grade your harrowing haunt through this nightmare, what would it be?" },
        new DialogueEntry { speakerName = "Female Orc", message = "Choose wisely - a misstep here could be... painfully regrettable." },
        new DialogueEntry { speakerName = "Player", message = "Hmmmm, Let me guess" },
    };
        StartCoroutine(TypeConversation(conversation));
    }
